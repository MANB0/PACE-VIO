...
def _load_timed_se3_data(file: Path) -> np.ndarray:
    path = Path(file)
    if path.suffix == ".npy":
...
def _resolve_timed_se3_file(box: Sandbox, stem: str) -> Path:
    for suffix in (".csv", ".npy", ".txt"):
        candidate = box.path(f"{stem}{suffix}")
        if candidate.exists():
            return candidate
    raise FileNotFoundError(f"Unable to load {stem}.csv/.npy/.txt from {box.folder}")
...
class Trajectory:
    ...
    def from_timed_SE3_numpy(cls, file: Path, frame_status: torch.Tensor | None = None) -> Trajectory:
        data = _load_timed_se3_data(file)
        timestamp, SE3_matrix = torch.tensor(data[:, 0]), pp.SE3(data[:, 1:])
        if frame_status is None:
            frame_status = torch.zeros((SE3_matrix.size(0),), dtype=torch.bool)
        return cls(SE3_matrix, timestamp, frame_status)
    ...
    def from_sandbox(cls, box: Sandbox, align_time: Literal["est->gt", "gt->est", None]="est->gt") -> tuple[Plotable[Trajectory], Plotable[Trajectory]]:
        ...
        """
        poses_file = _resolve_timed_se3_file(box, "poses")
        ref_poses_file = _resolve_timed_se3_file(box, "ref_poses")
        ...
        if flag_path.exists():
            ...
            frame_status = None
        est_traj = cls.from_timed_SE3_numpy(poses_file, frame_status=frame_status)
        gt_traj  = cls.from_timed_SE3_numpy(ref_poses_file)
    ...
    def from_sandbox_mayberef(cls, box: Sandbox) -> tuple[PlotableTrajectory | None, PlotableTrajectory]:
        if box.path("ref_poses.csv").exists() or box.path("ref_poses.npy").exists() or box.path("ref_poses.txt").exists():
        ...
        if flag_path.exists():
            ...
            frame_status = None
        est_traj = cls.from_timed_SE3_numpy(_resolve_timed_se3_file(box, "poses"), frame_status=frame_status)
