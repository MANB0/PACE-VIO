#!/usr/bin/env python3
"""Build an interactive comparison for VIO vector-state refinement."""
...
def write_comparison_manifest(
    *,
    ...
    old_rows = read_manifest(old_manifest)
    new_rows = read_manifest(new_manifest)
    result_root.mkdir(parents=True, exist_ok=True)

    rows: list[dict[str, str]] = []
    for scene in SCENES:
        specs = [
            (old_rows, "pure_macvo_baseline", "pure_macvo_baseline"),
            (old_rows, "imuatt_before_9x9", "imuatt_before_9x9"),
            (old_rows, "imuatt_after_9x9", "imuatt_before_vector_refine"),
            (new_rows, "vio_preintegrated_full_imuatt_estinit", "imuatt_vector_refine"),
        ]
...
def print_delta(summary_path: Path) -> None:
    with summary_path.open("r", encoding="utf-8", newline="") as f:
        rows = list(csv.DictReader(f))
    by_key = {(row["scene"], row["method"]): row for row in rows}

    print("\nVector-state refinement delta:")
    print(
        "scene,before_ate,new_ate,delta_ate,delta_pct,"
        "before_tvel,new_tvel,before_rvel,new_rvel"
    )
    for scene in SCENES:
        before = by_key[(scene, "imuatt_before_vector_refine")]
        after = by_key[(scene, "imuatt_vector_refine")]
        before_ate = float(before["ate_rmse_m"])
        after_ate = float(after["ate_rmse_m"])
        delta = after_ate - before_ate
        pct = delta / before_ate * 100.0 if before_ate else 0.0
        print(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--old-manifest",
        type=Path,
        default=WORKDIR / "Results/orbstyle_before_after_9x9_20260707/run_manifest.csv",
    )
    parser.add_argument(
        "--new-manifest",
        type=Path,
        default=WORKDIR / "Results/closed_paths_vector_refine_20260707/run_manifest.csv",
    )
    parser.add_argument(
        "--result-root",
        type=Path,
        default=WORKDIR / "Results/vector_refine_comparison_20260707",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=WORKDIR / "analysis_vector_refine_comparison_20260707",
    )
...
def main() -> int:
    ...
    if rc == 0:
        print_delta(args.output_root / "trajectory_summary.csv")
    return rc
