#!/usr/bin/env python3
"""
V3b murky ablation D2+D4 — sequential script generator

D2 = rerun current pair with full_imu on visual_collapse trigger
...
Output: Results/v3b_murky_ablation_d2d4_YYYYMMDD_HHMMSS/
"""
...
RUN_ROOT = PROJECT_ROOT / "Results" / f"v3b_murky_ablation_d2d4_{TIMESTAMP}"

# D2+D4: rerun on VC + sustain=1
...
def write_completed_marker(result_dir: Path, scene: str, trial: int,
                           return_code: int):
    """Write completed.ok with D2+D4 ablation logging fields."""
    # Read adaptive_decisions.csv for D2 verification fields
    d2_rerun_enabled = ""
    vc_sustain_config = ""
    rerun_triggered_count = 0
    first_rerun_pair = ""
    committed_result_source = "original"
    d2_rerun_failed_count = 0
    ...
    if ad_path.exists():
        try:
            ...
            with open(ad_path) as f:
                ...
                rows = list(reader)
            if rows:
                d2_rerun_enabled = rows[0].get("d2_rerun_enabled", "")
                vc_sustain_config = rows[0].get("visual_collapse_sustain_config", "")
                for r in rows:
                    if r.get("d2_rerun_triggered", "0") == "1":
                        rerun_triggered_count += 1
                        if not first_rerun_pair:
                            first_rerun_pair = r.get("pair_id", "")
                    if r.get("d2_committed_result_source", "") == "full_imu_rerun":
                        committed_result_source = "full_imu_rerun"
                    if r.get("d2_rerun_failed", "0") == "1":
                        d2_rerun_failed_count += 1
        except Exception:
            pass

    marker = result_dir / "completed.ok"
    marker.write_text(
        f"config_name=d2d4_rerun_on_vc_sustain1\n"
        f"scene={scene}\n"
        f"trial={trial}\n"
        ...
        f"D2_rerun_enabled={d2_rerun_enabled}\n"
        f"visual_collapse_sustain_config={vc_sustain_config}\n"
        f"rerun_current_pair_triggered_count={rerun_triggered_count}\n"
        f"first_rerun_pair={first_rerun_pair}\n"
        f"committed_result_source={committed_result_source}\n"
        f"d2_rerun_failed_count={d2_rerun_failed_count}\n"
    )
...
def run_one(scene: str, trial: int, result_root: Path,
    ...
    print(f"  Ablation: D2 (rerun on VC) + D4 (sustain=1)")
    print("─" * 60)
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            print(f"\n  ❌ FAILED {label}")
            print(f"     Config:  D2+D4 (rerun_on_vc + sustain=1)")
            print(f"     Scene:   {scene}")
            print(f"     Trial:   {trial}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="V3b murky ablation D2+D4 — sequential evaluation"
    )
    ...
    if args.dry_run:
        result_root = PROJECT_ROOT / "Results" / "v3b_murky_ablation_d2d4_DRYRUN"
    ...
    print("=" * 70)
    print("V3b murky ablation D2+D4 — Sequential Evaluation")
    print(f"  Ablation:  D2 (rerun current pair on VC) + D4 (sustain=1)")
    print(f"  Scene:     {SCENE}")
    ...
    print()
    print("=" * 70)
    print("V3b murky ablation D2+D4 — COMPLETE")
    print("=" * 70)
    print(f"Output: {result_root}")
    print()
    ...
    print()
    print("To analyze: tell the agent 'murky ablation D2+D4 run 已完成，开始 analyze.'")
