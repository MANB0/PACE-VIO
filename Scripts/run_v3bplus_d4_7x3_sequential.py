#!/usr/bin/env python3
"""
V3b+ D4 7×3 Sequential Evaluation — 7 scenes × 3 trials, strictly sequential.
...
D4 = visual_collapse_sustain 5 → 1 (no D2, no velocity reset).
...
Output: Results/v3bplus_d4_7x3_YYYYMMDD_HHMMSS/
"""
...
RUN_ROOT = PROJECT_ROOT / "Results" / f"v3bplus_d4_7x3_{TIMESTAMP}"

# V3b+ D4: sustain=1, all other thresholds at default V3b values.
...
def build_config(scene_root: Path, result_dir: Path) -> Path:
    """Create temporary odom.yaml + seq.yaml for one V3b+ D4 run."""
...
def write_completed_marker(result_dir: Path, scene: str, trial: int,
                           return_code: int):
    """Write completed.ok with V3b+ D4 ablation logging fields."""
...
def run_one(scene: str, trial: int, result_root: Path,
    ...
    print(f"  Config: V3b+ D4 (sustain=1, no D2)")
    print("─" * 60)
    ...
    #
    # Full output is visible in the terminal — user sees MACVO's tqdm
    # progress bars and status messages live.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="V3b+ D4 7×3 Sequential Evaluation — sustain=1"
    )
    ...
    if args.dry_run:
        result_root = PROJECT_ROOT / "Results" / "v3bplus_d4_7x3_DRYRUN"
    ...
    print("=" * 70)
    print("V3b+ D4 7×3 Sequential Evaluation")
    ...
    print()
    print("To analyze: tell the agent 'V3b+ D4 7×3 run 已完成，开始 analyze.'")
    print(f"Results directory: {result_root}")

    # ── Optionally run analysis ────────────────────────────────────
    print()
    print("Next step:")
    print("  python analysis_v3bplus_d4_7x3_report/analyze_v3bplus_d4.py")
