#!/usr/bin/env python3
"""Run current preintegrated VIO on the paired clear-circle HoloOcean data."""
...
SCENE_ROOTS = {
    "clear_circle_normal_noise": Path(
        "/mnt/e/文档/holoocean/code/recordings/batch_clear_circle_pair_20260704/"
        "normal_noise/clear_circle_path"
    ),
    "clear_circle_zero_noise": Path(
        "/mnt/e/文档/holoocean/code/recordings/batch_clear_circle_pair_20260704/"
        "zero_noise/clear_circle_path"
    ),
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--result-root",
        type=Path,
        default=WORKDIR / "Results" / "clear_circle_pair_vio_20260704",
    )
    parser.add_argument(
        "--variants",
        nargs="*",
        default=["vio_preintegrated_full"],
...
def main() -> int:
    ...
    print("=" * 78)
    print("  Clear-circle paired VIO run")
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="clear_circle_pair_vio_"))
