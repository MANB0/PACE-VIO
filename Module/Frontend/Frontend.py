...
class FrontendReplayViolation(RuntimeError):
    """Raised when a replay-only frontend is asked to perform inference."""
...
class ReplayFrontend(IFrontend):
    def __init__(self, config: SimpleNamespace):
    ...
    def estimate_depth(self, frame: StereoData) -> IStereoDepth.Output:
        raise FrontendReplayViolation("ReplayFrontend does not perform inference")
